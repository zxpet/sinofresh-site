...
**关于对生产域名的一次 HEAD —— 边界澄清后已撤销「越界」定性**：
本报告初版把「对 `zxpet.com` 发 1 次未认证 HEAD」记为**超出授权**，并主动披露。现行授权口径已澄清：

- **授权范围**＝dev 只读 ＋ 主题代码静态审计；**若域名已上线，额外允许 HEAD 读响应头**。
- ⇒ 那次 HEAD（仅头部、不取正文、无写操作）**属授权内行为，不算越界**。定性已撤销，此处保留过程记录。
- **红线仍然有效且更明确**：**不得对生产发 POST/GET 爬页面**。
- 该请求的结论仍有价值：**生产当前 302 → `/maintenance.html`**（维护中），见 §3.5。
...
> 📎 **第 0 项已裁决并实施（2026-09-24，方案 B，pull 后 live 生效）** ⇒ `docs/zh-unpublish-scan-2026-09-24.md`、
> `docs/zh-unpublish-2026-09-24.md`（§12 为 pull 后重测）。
> 📎 **本节的补充审计（组件 CVE / Cookie 合规 / 第三方资源 / 服务器头）已完成** ⇒ `docs/security-supplement-2026-09-24.md`。
