...
> **本批只读、零改动**；生产站未触碰。
>
> ⚠️ **2026-09-24 后续更正 —— 见 `docs/security-supplement-2-2026-09-24.md`。** 三处要改：
> ① 本节把 GF 的 `version higher than expected` 记成「更新检测坏了」——**错**，那是**更新源被改写**的症状；
> ② 「生产站必须重跑同一份前提检查」是**错框** —— 生产**根本没装 Gravity Forms**；
> ③ 因此「升级到 3.1.2」这个修法**指向了错的修复**（dev 那份 GF 不是厂商代码，自带更新指向第三方桶）。
> 本节正文**保持原样不改**，作为「当时掌握的线索」的存档；结论以后续那份为准。
...
| 结论 | 项 | 优先级 |
|---|---|---|
| 🔴 **Gravity Forms 3.1.0.3 落在 CVE-2026-84434（CVSS 9.8 未认证 RCE）的受影响区间内**，但**利用前提在本站实测不成立** | 8.1 | ~~P1（升级动作）~~ ⇒ **已被 P0 取代：该插件是破解版**（`docs/security-supplement-2-2026-09-24.md` §3） |
...
**处置（以下三条已被后续实测推翻，此处保留原文以便对照；现行结论见 `docs/security-supplement-2-2026-09-24.md` §3）**：
1. ~~**升级到 3.1.2**（≥3.1.1 即修）~~ ⇒ **不可执行**：dev 的 GF 是**破解版**（`GRAVITY_MANAGER_URL='https://gf.gpltimes.com'`、假 key 每次加载写库、`pre_http_request` 伪造 Elite 授权），自带更新指向**第三方 Backblaze 桶** ⇒ **不得用其自带更新**。正解＝买正版重装，或换插件。
2. ⚠️ ~~**生产站必须重跑同一份前提检查**~~ ⇒ **前提不成立**：生产是**另一套站**（`/var/www/html`，DB `wordpress`），**未装 GF**，连 `%gf_%` 表都没有 ⇒ **不存在「生产腿」**。
3. ⚠️ ~~**GF 的更新检测在本站是坏的**~~ ⇒ **误判**：`version higher than expected` 的含义是「接口报的最新版（2.2.6.5）**低于**已装版（3.1.0.3）」——这是**更新源被改写**的一手证据，不是 WP 的毛病。
