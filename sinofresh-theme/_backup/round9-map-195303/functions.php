...
add_action('wp_enqueue_scripts', function() {
	wp_enqueue_style('sinofresh-style', get_stylesheet_uri(), array(), '2.6.6');
...
add_action('wp_enqueue_scripts', function() {
	...
	if ($is_dosage_page) {
		wp_enqueue_style('sinofresh-configurator', get_template_directory_uri() . '/assets/css/configurator.css', array(), '1.3');
		wp_enqueue_script('sinofresh-configurator', get_template_directory_uri() . '/assets/js/configurator.js', array(), '1.2', true);
	}
});
	...
	add_menu_page(
		...
		function () {
			...
			<div class="wrap">
				...
				<p>Fill in the profile URLs below. The header top bar and the footer icons update automatically — leave a field empty (or “#”) to keep the icon pointing to “#”.</p>
...
add_action('wp_head', function () {
	...
	$schema = array(
		...
		'url'          => home_url('/'),
		'email'        => 'info@zxpet.com',
		'telephone'    => '+86-539-866-9539',
		'address'      => array(
			'@type'            => 'PostalAddress',
			'streetAddress'    => 'Building B3, No. 22 Zhongshan Road, Yihe New Area',
