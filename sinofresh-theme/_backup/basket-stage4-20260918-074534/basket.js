 ...
 *   Clear All (confirm) and Submit Inquiry. "Download Basket PDF" is a
 *   placeholder until stage 4 wires the multi-item PDF.
...
(function () {
	...
	function fillFromBasket() {
		...
		if (wrapper && !document.querySelector('.sf-basket-notice')) {
			...
			text.textContent = items.length + ' item' + (items.length > 1 ? 's' : '')
				+ ' from your inquiry basket have been pre-filled below.';
