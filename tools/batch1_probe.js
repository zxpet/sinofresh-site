/* Batch-1 probe for the 8 dosage pages.
 *
...
const PAGES = [
  ['soft-chews', '/products/soft-chews/', 'standard'],
  ['tablets', '/products/tablets/', 'standard'],
  ['powders', '/products/powders/', 'standard'],
  ['pastes', '/products/pastes/', 'standard'],
  ['drops', '/products/drops/', 'variant'],
  ['liquids', '/products/liquids/', 'variant'],
  ['fish-oil', '/products/fish-oil/', 'variant'],
  ['dental-chews', '/products/dental-chews/', 'variant'],
];
...
(async () => {
  ...
  await browser.close();
  const dest = path.resolve(__dirname, '..', 'screenshots', `batch1-${LABEL}.json`);
