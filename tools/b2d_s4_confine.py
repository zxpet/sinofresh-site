...
#
# This tool extracts the gallery band's heading and rebuilds it to predict the
# next one (H2_RE at line ~82, the comparison at line ~152, both keyed on
# '<h2 class="sf-gallery__title">A Closer Look at %s</h2>'). Batch H7a deleted
# that heading, so this tool now fails on any capture taken from H7a onward.
# That failure is expected and is not a regression.
#
