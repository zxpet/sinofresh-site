...
const PAGES = [
  ...
  ['fish-oil', '/products/fish-oil/'], ['dental-chews', '/products/dental-chews/'], ['quality', '/quality/'],
];
const SEO = () => {
...
(async () => {
  ...
  for (const [n, u] of PAGES) {
    ...
    flag(r.stepTitles.length >= 7, 'stepTitles=' + r.stepTitles.length);
    flag(r.relatedLinks.length >= 7, 'relatedLinks=' + r.relatedLinks.length);
