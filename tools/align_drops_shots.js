...
const OUT = '/Users/meng/Workbuddy/sinofresh外贸网站建设/screenshots/align-drops';
require('fs').mkdirSync(OUT, { recursive: true });
(async () => {
  const b = await chromium.launch({ channel: 'chrome' });
  const ctx = await b.newContext({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1 });
  const page = await ctx.newPage();
  await page.goto('http://sinofresh.local/products/drops/', { waitUntil: 'networkidle' });
  ...
  const cta = await page.evaluate(() => {
    ...
    return { x: 0, y: Math.round(r.y + window.scrollY) - 10, width: 1440, height: Math.min(Math.round(r.height) + 20, 1400) };
  });
  await page.screenshot({ path: path.join(OUT, `${tag}-1440-cta.png`), clip: { x: cta.x, y: cta.y, width: cta.width, height: cta.height }, fullPage: true });
  // hero
  ...
  await m.goto('http://sinofresh.local/products/drops/', { waitUntil: 'networkidle' });
